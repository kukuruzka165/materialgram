...
https://github.com/kukuruzka165/materialgram/blob/dev/LEGAL
*/
...
void ParseLanguageData(
	...
	if (!file.open(QIODevice::ReadOnly)) {
		LOG(("MaterialLang::Lang Info: file %1 could not be read.").arg(filename));
		return;
	}
	...
	if (error.error != QJsonParseError::NoError) {
		LOG(("MaterialLang::Lang Info: file %1 has failed to parse. Error: %2"
			).arg(filename
			).arg(error.errorString()));
		return;
	} else if (!document.isObject()) {
		LOG(("MaterialLang::Lang Info: file %1 has failed to parse. Error: object expected"
			).arg(filename));
		return;
	}
		...
		if ((*value).isString()) {
			...
			for (auto pli = pluralList.constBegin(), ple = pluralList.constEnd(); pli != ple; ++pli) {
				...
				if (!(*pluralValue).isString()) {
					LOG(("MaterialLang::Lang Info: wrong value for key %1 in %2 in file %3, string expected")
			...
			LOG(("MaterialLang::Lang Info: wrong value for key %1 in file %2, string or object expected")
